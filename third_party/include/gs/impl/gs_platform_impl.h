...
GS_API_DECL gs_result  gs_platform_write_file_contents(const char* file_path, const char* mode, void* data, size_t _sz)
{
    FILE* fp = fopen(file_path, mode);
    if (fp) 
    {
        size_t ret = fwrite(data, sizeof(uint8_t), _sz, fp);
        if (ret == _sz) {
            fclose(fp);
...
int32_t gs_platform_file_size_in_bytes(const char* file_path)
{
        ...
        HANDLE hFile = CreateFile(file_path, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if ( hFile == INVALID_HANDLE_VALUE )
		{
			gs_println("Error: %zu", GetLastError());
		...
		//_sopen_s(&fd, file_path, _O_RDONLY, _SH_DENYRW, _S_IREAD);
		//if (fd == -1) { 
		//	return -1;
		//}
